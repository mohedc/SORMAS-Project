	...
	public void refreshMenu(SubMenu menu, String params) {
		...
		if (showExtraMenuEntries) {
			...
			if (UiUtil.permitted(FeatureType.VIEW_TAB_CASES_THERAPY, UserRight.THERAPY_VIEW)
				...
				Disease[] excludedDiseases = { Disease.NEONATAL_TETANUS, Disease.MEASLES, Disease.YELLOW_FEVER, Disease.CONGENITAL_RUBELLA, Disease.IMMEDIATE_CASE_BASED_FORM_OTHER_CONDITIONS, Disease.CSM };
				if (!Arrays.asList(excludedDiseases).contains(caze.getDisease())) {
		...
		if (showExtraMenuEntries) {
			if (UiUtil.permitted(
				...
				Disease[] excludedDiseases = { Disease.MEASLES, Disease.YELLOW_FEVER, Disease.NEONATAL_TETANUS, Disease.AFP, Disease.CONGENITAL_RUBELLA, Disease.IMMEDIATE_CASE_BASED_FORM_OTHER_CONDITIONS, Disease.CSM };
				if (!Arrays.asList(excludedDiseases).contains(caze.getDisease())) {
					menu.addView(
