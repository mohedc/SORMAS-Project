...
public class LgaView extends AbstractConfigurationView {

	private static final long serialVersionUID = -3487830069266335042L;

	public static final String VIEW_NAME = ROOT_VIEW_NAME + "/lgas";

	private LgaCriteria criteria;
	...
	public LgaView() {
		...
		viewConfiguration = ViewModelProviders.of(LgaView.class).get(ViewConfiguration.class);
		criteria = ViewModelProviders.of(LgaView.class)
			.get(LgaCriteria.class, new LgaCriteria().country(FacadeProvider.getCountryFacade().getServerCountry()));
		if (criteria.getRelevanceStatus() == null) {
			criteria.relevanceStatus(EntityRelevanceStatus.ACTIVE);
		}
		grid = new LgaGrid(criteria);
		gridLayout = new VerticalLayout();
		gridLayout.addComponent(createFilterBar());
		rowCount = new RowCount(Strings.labelNumberOfLgas, grid.getDataSize());
		...
		if (UiUtil.permitted(infrastructureDataEditable, UserRight.INFRASTRUCTURE_IMPORT)) {
			importButton = ButtonHelper.createIconButton(Captions.actionImport, VaadinIcons.UPLOAD, e -> {
				Window window = VaadinUiUtil.showPopupWindow(new InfrastructureImportLayout(InfrastructureType.LGA));
				window.setCaption(I18nProperties.getString(Strings.headingImportLgas));
		...
		if (UiUtil.permitted(UserRight.INFRASTRUCTURE_EXPORT)) {
			...
			StreamResource streamResource = GridExportStreamResource
				.createStreamResourceWithSelectedItems(grid, this::getSelectedRows, ExportEntityName.LGAS, LgaGrid.ACTION_BTN_ID);
		...
		if (UiUtil.permitted(infrastructureDataEditable, UserRight.INFRASTRUCTURE_CREATE)) {
			createButton = ButtonHelper.createIconButton(
				...
				e -> ControllerProvider.getInfrastructureController().createLga(),
				ValoTheme.BUTTON_PRIMARY);
	...
	private Set<LgaIndexDto> getSelectedRows() {
		LgaGrid lgaGrid = this.grid;
		return this.viewConfiguration.isInEagerMode() ? lgaGrid.asMultiSelect().getSelectedItems() : Collections.emptySet();
	}
	...
	private HorizontalLayout createFilterBar() {
		...
		resetButton = ButtonHelper.createButton(Captions.actionResetFilters, event -> {
			ViewModelProviders.of(LgaView.class).remove(LgaCriteria.class);
			navigateTo(null);
		...
		actionButtonsLayout.setSpacing(true);
		{
			...
			if (UiUtil.permitted(UserRight.INFRASTRUCTURE_VIEW)) {
				...
				relevanceStatusFilter.setItemCaption(EntityRelevanceStatus.ACTIVE, I18nProperties.getCaption(Captions.lgaActiveLgas));

				if (UiUtil.permitted(UserRight.INFRASTRUCTURE_VIEW_ARCHIVED)) {
					relevanceStatusFilter.setItemCaption(EntityRelevanceStatus.ARCHIVED, I18nProperties.getCaption(Captions.lgaArchivedLgas));
					relevanceStatusFilter
						.setItemCaption(EntityRelevanceStatus.ACTIVE_AND_ARCHIVED, I18nProperties.getCaption(Captions.lgaAllLgas));
				} else {
