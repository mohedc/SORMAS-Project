...
public class SampleDtoHelper extends AdoDtoHelper<Sample, SampleDto> {
	...
	public void fillInnerFromDto(Sample target, SampleDto source) {
		...
		target.setSampleContainerUsedOther(source.getSampleContainerUsedOther());
		target.setMeningitisRdtPerformed(source.getMeningitisRdtPerformed());
		target.setMeningitisRdtResult(source.getMeningitisRdtResult());
		target.setLabNumber(source.getLabNumber());
		target.setSampleContainerReceived(source.getSampleContainerReceived());
		target.setSampleContainerReceivedOther(source.getSampleContainerReceivedOther());
		target.setSampleConditionAtReception(source.getSampleConditionAtReception());
		target.setCsfAppearanceAtCollection(source.getCsfAppearanceAtCollection());
		target.setCsfAppearanceAtReception(source.getCsfAppearanceAtReception());
