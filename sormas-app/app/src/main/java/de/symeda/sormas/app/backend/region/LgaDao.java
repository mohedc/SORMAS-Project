...
public class LgaDao extends AbstractInfrastructureAdoDao<Lga> {

	public LgaDao(Dao<Lga, Long> innerDao) {
		super(innerDao);
	}
	...
	public String getTableName() {
		return Lga.TABLE_NAME;
	}

	@Override
	public Lga saveAndSnapshot(Lga source) {
		throw new UnsupportedOperationException();
	}

	public List<Lga> queryActiveByServerCountry() {
		...
		try {
			QueryBuilder<Lga, Long> builder = queryBuilder();
			Where<Lga, Long> where = builder.where();
			where.and(where.eq(AbstractDomainObject.SNAPSHOT, false), where.eq(InfrastructureAdo.ARCHIVED, false));

			if (serverCountry != null) {
				where.and().eq(Lga.COUNTRY + "_id", serverCountry).or().isNull(Lga.COUNTRY + "_id");
			}

			return builder.orderBy(Lga.NAME, true).query();
	...
	public List<Lga> queryActiveByCountry(Country country) {
		try {
			QueryBuilder<Lga, Long> builder = queryBuilder();
			Where<Lga, Long> where = builder.where();
			where.and(
				...
				where.eq(Lga.COUNTRY + "_id", country));
	...
	public List<Lga> queryActiveByArea(Area area) {
		try {
			QueryBuilder<Lga, Long> builder = queryBuilder();
			Where<Lga, Long> where = builder.where();
			where.and(
